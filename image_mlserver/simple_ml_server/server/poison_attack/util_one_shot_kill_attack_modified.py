...
def get_feat_rep(x):
    ...
    res = np.squeeze(np.array(sess.run(feat_tensor, feed_dict={input_tensor:x})))
...
def load_bottleNeckTensor_data(directory=None, saveEm=False, random_state=123, train_size=400):
    ...
    """
    #some parameters 
    directorySaving = './Test_Data/XY/' #directory to save the X and Ys
...
def append_train_data(sess, X_tr, Y_tr, newInpImage, newInpClass):
    ...
    newFeatRep = np.expand_dims(np.squeeze(sess.run(feat_tensor, feed_dict={input_tensor: newInpImage})), axis=0)
    
    #append the training data and add the poison to the end of it
    X_tr = np.vstack((X_tr,newFeatRep))
...
def test(sess, targetInpImage, expectedClass, class_identifier="dog_and_fish"):
    ...
    class_A_B = {
        ...
        "mouse_and_cat": {"mouse":np.array([[1., 0.]]), "cat":np.array([[0., 1.]])}
    }
    ...
    print('Expected Result:',expectedClass)

    return target_corr_pred

def train_last_layer_of_inception(X_tr,Y_tr,Y_validation,X_validation):
    """
    ...
    #we are doing cold start, load the appropriate graph
    saver = tf.train.import_meta_graph('./server/poison_attack/dog_v_fish_cold_graph/dog_v_fish_cold_graph.meta')
    saver.restore(sess,tf.train.latest_checkpoint('./server/poison_attack/dog_v_fish_cold_graph/'))
